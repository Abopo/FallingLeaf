...
namespace MeshDistort
{
    ...
    public class Distort : MonoBehaviour
    {
        ...
        public bool calculateInGPU = true;
        ...
        void OnEnable()
        {
            ...
            if (calculateInGPU && distortShader != null && Application.isPlaying)
            {
                if (!SystemInfo.supportsComputeShaders)
                {
                    Debug.LogError("Platform does not support compute shaders");
                }
                else
                {
                    dirtortKernel = distortShader.FindKernel("Distort");
                    SetBuffers();
                } 
            }
        ...
        void OnDisable()
        {
            //Set the vertices to the original position
            ResetVertices();
            ReleaseBuffers();
        }
        ...
        public void SetBuffers()
        {
            ...
            {
                meshList[i].CreateBuffers();
            }
            for (int i = 0; i < distort.Count; i++)
            {
                distort[i].SetupBuffers();
            }
        }
        public void ReleaseBuffers()
        {
            ...
            for (int i = 0; i < meshList.Count; i++)
            {
                meshList[i].ReleaseBuffers();
            }

            if(distort != null)
            for (int i = 0; i < distort.Count; i++)
            {
                distort[i].ReleaseBuffers();
            }
        }
        ...
        {
            if(calculateInGPU && Application.isPlaying)
            for (int i = 0; i < distort.Count; i++)
            {
                distort[i].UpdateObjectDataBuffer();
                distort[i].UpdateBufferCurves();
            }
        }
        ...
        public void UpdateDistort()
        {
            ...
            if (calculateInGPU && Application.isPlaying && SystemInfo.supportsComputeShaders)
            {
                if (distortShader != null)
                    UpdateInGPU();
                else
                    Debug.LogError("Script is set to calculate in GPU, but no shader is selected.", this);
            }
            else
            {
                UpdateInCPU();
        ...
        public void UpdateInGPU()
        {
            ...
            foreach (MeshDistortData data in meshList)
            {
                ...
                globalToLocal = data.worldToLocalMatrix;

                
                /*
                if (data.skin != null && data.skin.rootBone != null)
                {
                ...
                */
                ...
                data.verticeBuffer.SetData(vertices);

                data.matrixBuffer.SetData(new Matrix4x4[]
                {
                localToGlobal, globalToLocal
                });
                data.BufferSet(distortShader,dirtortKernel);
                ...
                foreach (DistortData dist in distort)
                {
                    ...
                    if (dist.enabled)
                    {
                        ...
                        dist.UpdateFrameDataBuffer();

                        dist.bufferManager.SetBuffers(distortShader, dirtortKernel);

                        distortShader.Dispatch(dirtortKernel, Mathf.CeilToInt(vertices.Length / 64f), 1, 1);
                ...
                data.verticeBuffer.GetData(vertices);
                ...
                for (int i = 0; i < verLength; i++)
                {
                    vertices[i] = globalToLocal.MultiplyPoint3x4(vertices[i]);
                }
                */
