...
namespace MeshDistort
{
    ...
    public class DistortEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            myTarget.calculateInGPU = GUILayout.Toggle(myTarget.calculateInGPU, new GUIContent("Calculate in GPU", "Do the heavy calculations in the GPU."));
            ...
            if (distort != null)
            {
                for (int i = 0; i < distort.Count; i++)
                {
                    ...
                    if (distort[i].showInEditor)
                    {
                        ...
                        EditorGUILayout.BeginVertical("box");
                        //Stretch and random have different options
                        if (distort[i].type == Distort.Type.Stretch || distort[i].type == Distort.Type.Random || distort[i].type == Distort.Type.Melt)
                        {
                        ...
                        {
                            EditorGUILayout.LabelField(new GUIContent("Displaced Force:", "Calculated WITH the Force Displacement value."));
                            distort[i].displacedForceX = EditorGUILayout.CurveField(new GUIContent("X", "Displacement in the X axis"), distort[i].displacedForceX);
                            distort[i].displacedForceY = EditorGUILayout.CurveField(new GUIContent("Y", "Displacement in the Y axis"), distort[i].displacedForceY);
                            distort[i].displacedForceZ = EditorGUILayout.CurveField(new GUIContent("Z", "Displacement in the Z axis"), distort[i].displacedForceZ);
                        }
